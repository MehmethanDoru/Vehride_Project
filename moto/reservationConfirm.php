...
if (!empty($_POST)) {
    ...
    $sql = "INSERT INTO `moto_rent_details` (`unique_id`, `fullName`, `birthday_date`, `identity_number`, `licence_number`, `address`, `phone_number`, `phone_number2`, `email`, `city_id`, `brand_id`, `model_id`, `pickup_date`, `dropoff_date`) VALUES ('$randomString', '$fullName', '$birthday', '$identity', '$license', '$address', '$phone', '$phone2', '$email', '$city', '$brand', '$model', '$pickupDate', '$dropoffDate')";

    $result = mysqli_query($conn, $sql);


    $sql = "UPDATE `day_of_year_moto` SET rent=CONCAT(rent,',$model.$city') WHERE dates BETWEEN '$pickupDate' AND '$dropoffDate'";
    $result = mysqli_query($conn, $sql);
