...
if (!empty($_SESSION['pickupDate'])) {
    ...
    $sql = "SELECT * FROM `moto_model` WHERE id = '$model'";
    $result = mysqli_query($conn, $sql);

    $moto = array();
    if (mysqli_num_rows($result) > 0) {
        $moto = mysqli_fetch_all($result, MYSQLI_ASSOC);
    }

    $totalPrice = $moto[0]['price_daily'] * $daysDiff;

    $sql = "SELECT `brand_name` FROM `moto_brands` WHERE id = '$brand'";
    $result = mysqli_query($conn, $sql);
    ...
    $motoModel = array();
    if (mysqli_num_rows($result) > 0) {
        $motoModel = mysqli_fetch_all($result, MYSQLI_ASSOC);
    }
    ...
    $sql = 'SELECT * FROM `moto_brands` ORDER BY brand_name ASC';
    $result = mysqli_query($conn, $sql);
...
<head>
    ...
    <title>Reservation | VEHRIDE</title>
...
<body>
    ...
    <?php if (isset($_SESSION['pickupDate'])) { ?>
        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-lg-4 col-xs-8" id="motoImg">
                    <img src="../images/moto/moto_models/<?php echo $moto[0]['moto_image']; ?>"
                        alt="<?php echo $moto[0]['model_name'] ?>" width="100%">
                </div>
                <div class="col-lg-5 col-xs-6" id="motoInfo">
                    <div class="motoInfo">
                        <h5><strong>Brand Name:</strong>
                            <?php echo $brand[0]['brand_name']; ?>
                        </h5><br>
                        <h5><strong>Model Name:</strong>
                            <?php echo $moto[0]['model_name']; ?>
                        </h5><br>
                        <h5><strong>Year of Model:</strong>
                            <?php echo $moto[0]['model_year']; ?>
                        </h5><br>
                        <h5><strong>Price Daily:</strong>
                            <?php echo $moto[0]['price_daily']; ?>&#8364;
                        </h5><br>
        ...
        <div class="container mt-4 mb-5" id="step2" style="display:none;">
            <div class="row">
                <form action="reservationConfirm.php" method="post">
                    ...
                    <div id="btnConfirmDiv"><button type="submit" class="" id="rezConfirm">Reservation Confirm</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <?php } else { ?>
        <div class="motosShow">
            <?php foreach ($motoModel as $motosModel) {
                echo "<div id='model'><img height='150' width='270' src='../images/moto/moto_models/$motosModel[moto_image]'></div>";
            } ?>
        </div>
        <div class="container">
            <div class="row">
                <h5 class='mt-3' style="text-align: center">Enter your reservation number for to query</h5>
                <div class="rezQuery">
            ...
            <div class="row mx-auto">
                ...
                <div class="col-sm-8 offset-sm-2 offset-md-1 col-md-6 short-rental mt-5">
                    <div class="formCenter py-3">
                        <form id="brandForm" method="post" action="motoAvailabilityControl.php">
                            ...
                            <div class="brand-model">
                                <div class="brand-container">
                                    <div class="brand">
                                        <label for="brand">Brand of Motorcycle:</label><br>
                            ...
                            <div class="btn-container mb-2 pb-2">
                                <button type="submit" class="btnShort" name="brandForm">Search Motorcycle</button>
                            </div>
                        </form>
                    </div>
                </div>
